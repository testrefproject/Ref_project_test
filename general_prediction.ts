...
function sqlDDL(options: {
  ...
  def create_topic(context):
      ...
      topic_lst = [topic.strip(":") for topic in topics[0]['text'].split(",")]
      topic_lst = [topic.strip() for topic in topic_lst]
      topic_lst = [topic for topic in topic_lst if topic!=""]
  ...
  for feed in feeds:
      ...
      pred_dct[str(feed['title'])] = json_data
