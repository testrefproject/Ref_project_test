  ...
  return SQLa.SQL<EmitContext>(dvts.ddlOptions)`
    ...
    ${ schemaName
       ...
       : "-- no schemaName provided" }
        ...
        CREATE OR REPLACE FUNCTION sentence_transformer()
  RETURNS text
  LANGUAGE plpython3u
 AS $function$
...
model_name = "Sentence Transformers Embeddings"
...
# Fetch entries
entries_query = "SELECT title, content FROM fdw_stateful_service_miniflux.entries ORDER BY created_at DESC LIMIT 30;"
...
for index, row in df_n.iterrows():
    ...
    feed_title_topic[index] = result[0]
    pred_score_title[index] = result[1]

    if len(feed_content) > 0:
        feed_title_emb = embeddings.embed_query(feed_content)
        result = topic_matcher(df_t, feed_title_emb, 10)
        feed_content_topic[index] = result[0]
        pred_score_content[index] = result[1]
    else:
        result = "", ""
        feed_content_topic[index] = result
        pred_score_content[index] = result

    if len(feed_content) > 0:
        title_contentfeed = feed_title + feed_content
        feed_title_emb = embeddings.embed_query(title_contentfeed)
        result = topic_matcher(df_t, feed_title_emb, 10)
        feed_title_content[index] = result[0]
        pred_score_title_content[index] = result[1]
    else:
        result = "", ""
        feed_title_content[index] = result
        pred_score_title_content[index] = result
...
for index, row in df_n.iterrows():
    ...
    insert_values = (feed_title, feed_content, feed_title_and_content, model_name, json.dumps(feed_title_topic.get(index, "")), json.dumps(pred_score_title.get(index, "")), json.dumps(feed_content_topic.get(index, "")), json.dumps(pred_score_content.get(index, "")), json.dumps(feed_title_content.get(index, "")), json.dumps(pred_score_title_content.get(index, "")))
